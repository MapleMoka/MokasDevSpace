...
namespace MFM
{
    public class JobDriver_HarvestFromPawn : JobDriver_GatherAnimalBodyResources
    {
        private float gatherProgress;

        protected override float WorkTotal => 400f;

        protected override CompHasGatherableBodyResource GetComp(Pawn animal) => (CompHasGatherableBodyResource)ThingCompUtility.TryGetComp<MFM.Comp_HarvestFromGene>((Thing)animal);

        protected override IEnumerable<Toil> MakeNewToils()
        {
            this.FailOnDespawnedNullOrForbidden<JobDriver_HarvestFromPawn>(TargetIndex.A);
            this.FailOnDowned<JobDriver_HarvestFromPawn>(TargetIndex.A);
            this.FailOnNotCasualInterruptible<JobDriver_HarvestFromPawn>(TargetIndex.A);
