...
namespace MFM
{
    public class JobDriver_PluckPawn : JobDriver_GatherAnimalBodyResources
    {
        private float gatherProgress;

        protected override float WorkTotal => 400f;

        protected override CompHasGatherableBodyResource GetComp(Pawn animal) => (CompHasGatherableBodyResource)animal.TryGetComp<CompPluckableGene>();

        protected override IEnumerable<Toil> MakeNewToils()
        {
            this.FailOnDespawnedNullOrForbidden<JobDriver_PluckPawn>(TargetIndex.A);
            this.FailOnDowned<JobDriver_PluckPawn>(TargetIndex.A);
            this.FailOnNotCasualInterruptible<JobDriver_PluckPawn>(TargetIndex.A);
            ...
            {
                Pawn actor = wait.actor;
                actor.skills.Learn(SkillDefOf.Plants, 0.13f);
                this.gatherProgress += actor.GetStatValue(StatDefOf.PlantWorkSpeed);
            ...
            wait.activeSkill = (Func<SkillDef>)(() => SkillDefOf.Plants);
            yield return wait;
        }
    }
}
